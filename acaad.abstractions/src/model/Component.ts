  ...
  static fromMetadata(metadata: Chunk.Chunk<AcaadPopulatedMetadata>): Option<Component> {
    ...
    switch (first.component.type) {
      case ComponentType.Button:
        return some(
          ButtonComponent.fromMetadataInternal(first.component.name, first.serverMetadata, metadata)
        );
      case ComponentType.Sensor:
        return some(
          SensorComponent.fromMetadataInternal(first.component.name, first.serverMetadata, metadata)
        );
      case ComponentType.Switch:
        return some(
          SwitchComponent.fromMetadataInternal(first.component.name, first.serverMetadata, metadata)
        );
      default:
        return none();
    }
